# -*- coding: utf-8 -*-
"""CoLab_TermProj_2025_1.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/17EAOwawXN6URumRNqT0WvXexDiWUL0MJ
...
cnts['clip_enrichment'] = np.log2(cnts['CLIP-35L33G.bam'] / cnts['RNA-control.bam'])
cnts['rden_change'] = np.log2((cnts['RPF-siLin28a.bam'] / cnts['RNA-siLin28a.bam']) / (cnts['RPF-siLuc.bam'] / cnts['RNA-siLuc.bam']))
cnts.head()

cnts_clean = cnts[cnts[['clip_enrichment', 'rden_change']].applymap(np.isfinite).all(axis=1)]

plt.figure(figsize=(6,6))
sns.scatterplot(data=cnts_clean, x='clip_enrichment',y='rden_change',s=5, alpha=0.4, edgecolor=None, color='black')
...
cnts_clean.index = cnts_clean.index.str.split('.').str[0]
mouselocal_indexed = mouselocal.set_index('gene_id')
cnts_merged = cnts_clean.join(mouselocal_indexed, how='inner')

plt.figure(figsize=(6,6))
sns.scatterplot(data=cnts_merged, x='clip_enrichment',y='rden_change', hue='type', palette={'nucleus' : 'blue', 'integral membrane' : 'red', 'cytoplasm' : 'green'}, s=4, alpha=0.4, edgecolor=None)
